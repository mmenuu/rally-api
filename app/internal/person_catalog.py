...
class PersonCatalog:
    ...
    def get_person_by_id(self, user_id: str):
        return next((user for user in self.get_persons() if user.get_id() == user_id), None)

    # Setters
    def add_person(self, user: User | Admin):
        self.__users.append(user)

    def remove_person(self, user: User | Admin):
        self.__users.remove(user)
    
    def get_persons_by_keyword(self, keyword: str):
